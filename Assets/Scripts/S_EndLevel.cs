...
public class S_EndLevel : MonoBehaviour {
	...
	void FixedUpdate()
	{
		...
		if (Done && Input.GetKey(KeyCode.A) && !ALock) {
			GameObject.FindGameObjectWithTag ("StartLevel").GetComponent<S_LevelStart>().resetlevel(ref InpList);
			Done = false;
		}
		if (Done && Input.GetKey (KeyCode.S) && !SLock) {
			Done = false;
			GameObject.FindGameObjectWithTag ("StartLevel").GetComponent<S_LevelStart>().resetlevel();
		}
		ALock = Input.GetKey(KeyCode.A);
		SLock = Input.GetKey(KeyCode.S);
	}
