...
public class S_PlayerMovement : MonoBehaviour {
	...
	public Transform CheckpointReference;
	...
	void boosting()
	{
		...
		for (int i = 0; i < collider.Length; i++)
		{
			if (collider[i].gameObject.tag == "Boostable")
				Boostable = true;
		};
		if (Boostable) {
	...
	private void undeath()
	{
		if (Dying) {
			...
			Rigidbody.isKinematic = false;
			gameObject.transform.position = new Vector3(CheckpointReference.transform.position.x,CheckpointReference.transform.position.y,CheckpointReference.transform.position.z);
	...
	public void initlevel(ref List<byte> inp)
	{
